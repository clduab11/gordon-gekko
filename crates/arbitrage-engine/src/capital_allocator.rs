...
use crate::{AllocationRequest, AllocationPriority, ArbitrageError, ArbitrageResult};
use exchange_connectors::{Balance, ExchangeConnector, ExchangeId, TransferRequest, TransferUrgency};
...
impl CapitalAllocator {
    ...
    pub async fn initialize(&self) -> ArbitrageResult<()> {
        info!("💰 Initializing capital allocator across {} exchanges", self.exchanges.len());
        ...
        for (exchange_id, connector) in &self.exchanges {
            match connector.get_balances().await {
                Ok(exchange_balances) => {
                    info!("💵 Loaded {} currency balances from {:?}", 
                          exchange_balances.len(), exchange_id);
                ...
                Err(e) => {
                    ...
                    return Err(ArbitrageError::Exchange(format!(
                        "Failed to initialize balances for {:?}: {}", exchange_id, e
                    )));
                }
            }
        }
    ...
    pub async fn request_allocation(
        ...
        let request = AllocationRequest {
            ...
            deadline: chrono::Utc::now() + chrono::Duration::minutes(match priority {
        ...
        info!("🎯 Capital allocation requested: {} {} from {:?} to {:?} (Priority: {:?})",
    ...
    pub async fn emergency_reallocation(
        ...
        error!("Target: {:?}, Currency: {}, Percentage: {}%", target_exchange, currency, percentage * 100.0);
        ...
        for (exchange_id, exchange_balances) in balances.iter() {
            ...
            if let Some(balance) = exchange_balances.iter().find(|b| b.currency == currency) {
                let transfer_amount = balance.available * Decimal::from_f64_retain(percentage).unwrap_or_default();
                
                if transfer_amount > Decimal::ZERO {
                    let request_id = self.request_allocation(
                        *exchange_id,
                        target_exchange,
                        currency.clone(),
                        transfer_amount,
                    ...
                    error!("💀 Emergency transfer: {} {} from {:?} to {:?}", 
                           transfer_amount, currency, exchange_id, target_exchange);
                }
            }
        }

        error!("🎯 Emergency reallocation complete: {} transfers initiated", allocation_requests.len());
    ...
    async fn process_pending_allocations(&self) -> ArbitrageResult<()> {
        ...
        for request in sorted_requests {
            ...
            match self.execute_allocation(&request).await {
                Ok(_) => {
                    info!("✅ Allocation executed: {} {} from {:?} to {:?}", 
    ...
    async fn execute_allocation(&self, request: &AllocationRequest) -> ArbitrageResult<()> {
        ...
        if let Some(from_balances) = balances.get(&request.from_exchange) {
            if let Some(balance) = from_balances.iter().find(|b| b.currency == request.currency) {
        ...
        if let Some(from_connector) = self.exchanges.get(&request.from_exchange) {
            ...
            match from_connector.transfer_funds(transfer_request).await {
                Ok(transfer_id) => {
                    info!("💸 Transfer initiated: {} ({})", transfer_id, request.reason);
                    Ok(())
                }
                Err(e) => {
                    Err(ArbitrageError::Exchange(format!("Transfer failed: {}", e)))
                }
            }
        } else {
            Err(ArbitrageError::Configuration(
                format!("Exchange connector not found: {:?}", request.from_exchange)
            ))
        }
    }
    ...
    async fn perform_strategic_rebalancing(&self) -> ArbitrageResult<()> {
        ...
        match &*strategy {
            ...
            AllocationStrategy::Weighted(weights) => {
                // Use custom weights
                debug!("🎯 Weighted rebalancing strategy active with {} weights", weights.len());
            }
        }
    ...
    async fn calculate_target_allocations(&self) -> ArbitrageResult<()> {
        ...
        match &*strategy {
            AllocationStrategy::Balanced => {
                let per_exchange = Decimal::from_f64_retain(1.0 / total_exchanges).unwrap_or_default();
            ...
            AllocationStrategy::Aggressive => {
                ...
                for exchange_id in self.exchanges.keys() {
                    targets.insert(*exchange_id, Decimal::from_f64_retain(0.4).unwrap_or_default());
                }
            }
            AllocationStrategy::Weighted(weights) => {
                for (exchange_id, weight) in weights {
                    targets.insert(*exchange_id, Decimal::from_f64_retain(*weight).unwrap_or_default());
                }
            }
        }
