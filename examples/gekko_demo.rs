...
use exchange_connectors::{ExchangeId, ExchangeConnector};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   • Aggression Level: {:.0}% 💀", config.allocation_aggressiveness * 100.0);
    println!("   • Scan Frequency: {}ms ⚡", config.scan_frequency_ms);
    println!("   • Target Returns: {}:1 to {}:1 🎯", config.target_return_min, config.target_return_max);
    ...
    println!("   • Total Opportunities: {}", metrics.total_opportunities_detected);
